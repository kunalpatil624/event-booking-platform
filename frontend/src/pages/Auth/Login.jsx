            ...
            <motion.div className="grid grid-cols-2 max-md:grid-cols-1 w-full max-w-[960px] bg-bg-card border border-border-default rounded-3xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] relative z-[2]" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                ...
                <div className="p-12 px-10 max-md:p-9 max-md:px-6 flex flex-col justify-center">
                    ...
                    <p className="text-center text-text-secondary text-sm">
                        ...
                        <button className="bg-none border-none text-primary-light font-semibold cursor-pointer text-sm" onClick={() => setIsLogin(!isLogin)}>{isLogin ? 'Sign Up' : 'Login'}</button>
                    </p>
                    <div className="flex items-center gap-3 text-text-muted text-xs mt-4"><div className="flex-1 h-px bg-border-default" /><span>other portals</span><div className="flex-1 h-px bg-border-default" /></div>
                    <div className="flex gap-3 justify-center mt-2">
                        <Link to="/vendor" className="text-accent-emerald text-sm font-medium">🏢 Venue Owner Login</Link>
                        <Link to="/admin" className="text-red-500 text-sm font-medium">🛡️ Admin Login</Link>
                    </div>
                </div>
            </motion.div>
        </div>
