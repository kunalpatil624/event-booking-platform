...
export default function AdminLogin() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-10 px-6 relative overflow-hidden">
            ...
            <motion.div className="w-full max-w-[460px] bg-bg-card border border-border-default rounded-3xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] relative z-[2] p-10 max-md:p-8 max-md:px-6" initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6 }}>
                <div className="w-16 h-16 flex items-center justify-center rounded-2xl bg-red-500/15 text-red-500 text-3xl mx-auto mb-5"><HiShieldCheck /></div>
                ...
                {error && (<motion.div className="p-3 mb-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm text-center" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>{error}</motion.div>)}
                <form onSubmit={handleSubmit} className="flex flex-col gap-3.5 mb-5">
                    <div className={inputCls}><HiMail className="text-text-muted text-lg shrink-0" /><input type="email" placeholder="Admin Email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required className={inputInner} /></div>
                ...
                <div className="flex justify-center gap-6 text-sm">
                    <Link to="/vendor" className="text-text-secondary hover:text-white transition-all duration-300">Login as Venue Owner →</Link>
                    <Link to="/login" className="text-text-secondary hover:text-white transition-all duration-300">Login as User →</Link>
                </div>
            </motion.div>
        </div>
    );
