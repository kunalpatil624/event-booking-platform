        ...
        >
            <div className="max-w-[1280px] mx-auto px-6 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center gap-2">
                    <Link href="/venues" className="w-10 h-10 flex items-center justify-center rounded-full text-text-secondary text-lg hover:text-white hover:bg-white/[0.08] transition-all duration-300">
                        <HiSearch />
                    </Link>
                    <Link href="/wishlist" className="w-10 h-10 flex items-center justify-center rounded-full text-text-secondary text-lg hover:text-white hover:bg-white/[0.08] transition-all duration-300">
                        <HiHeart />
                    </Link>
                    <Link href="/dashboard" className="w-10 h-10 flex items-center justify-center rounded-full text-text-secondary text-lg hover:text-white hover:bg-white/[0.08] transition-all duration-300">
                        <HiCalendar />
                    </Link>
                    ...
                    {user ? (
                        ...
                        <Link href="/login" className="ml-2 inline-flex items-center gap-2 px-[18px] py-2 text-sm font-semibold rounded-xl bg-gradient-to-r from-primary to-primary-light text-white shadow-[0_4px_15px_rgba(108,60,225,0.4)] hover:-translate-y-0.5 hover:shadow-[0_6px_25px_rgba(108,60,225,0.5)] transition-all duration-300">
                            <HiUser /> Login
                        </Link>
                    )}
                </div>
                ...
                {menuOpen && (
                    ...
                    >
                        {[
                            ...
                            { href: '/venues', label: 'Venues' },
                            { href: '/venues?featured=true', label: 'Featured' },
                            { href: '/vendor', label: 'List Your Venue' },
                            { href: '/about', label: 'About' },
                        ].map(link => (
                            <Link key={link.href} href={link.href} className="py-3.5 px-4 text-base font-medium text-text-secondary rounded-xl hover:bg-white/5 hover:text-white transition-all duration-300" onClick={() => setMenuOpen(false)}>
                                {link.label}
                            </Link>
                        ))}
                        ...
                        {user ? (
                            <>
                                <div className="flex items-center gap-3 py-3.5 px-4 text-white font-medium">
                                    <div className="w-[34px] h-[34px] rounded-full bg-gradient-to-br from-primary to-primary-light flex items-center justify-center text-white font-bold text-sm">
                                        {user.name?.charAt(0).toUpperCase()}
                                    </div>
                                    <span>{user.name}</span>
                            ...
                            <Link href="/login" className="w-full mt-2 inline-flex items-center justify-center gap-2 px-7 py-3 text-base font-semibold rounded-xl bg-gradient-to-r from-primary to-primary-light text-white shadow-[0_4px_15px_rgba(108,60,225,0.4)]" onClick={() => setMenuOpen(false)}>
                                <HiUser /> Login / Sign Up
                            </Link>
                        )}

                        <Link href="/vendor" className="py-3.5 px-4 text-base font-medium text-text-secondary rounded-xl hover:bg-white/5 hover:text-white transition-all duration-300" onClick={() => setMenuOpen(false)}>
                            <HiOfficeBuilding className="inline mr-2" /> Venue Owner
                        </Link>
                        <Link href="/admin" className="py-3.5 px-4 text-base font-medium text-text-secondary rounded-xl hover:bg-white/5 hover:text-white transition-all duration-300" onClick={() => setMenuOpen(false)}>
