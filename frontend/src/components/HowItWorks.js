            ...
            <div className="max-w-[1280px] mx-auto px-6">
                ...
                <div className="grid grid-cols-4 max-lg:grid-cols-2 max-sm:grid-cols-1 gap-6 relative">
                    {steps.map((step, i) => (
                        <motion.div
                            key={i}
                            className="group text-center p-8 px-6 bg-bg-card border border-border-default rounded-2xl relative transition-all duration-300 hover:border-primary/30 hover:-translate-y-1.5 hover:shadow-[0_8px_32px_rgba(0,0,0,0.5)]"
                            initial={{ opacity: 0, y: 40 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            transition={{ duration: 0.5, delay: i * 0.15 }}
                        >
                            <div className="text-[2.5rem] font-black opacity-15 absolute top-4 right-5 font-display" style={{ color: step.color }}>
                                {step.step}
                            </div>
                            <div className="w-[72px] h-[72px] flex items-center justify-center rounded-[20px] text-[1.75rem] mx-auto mb-5 transition-all duration-300 group-hover:scale-110" style={{ background: `color-mix(in srgb, ${step.color} 12%, transparent)`, color: step.color }}>
                                {step.icon}
                            </div>
