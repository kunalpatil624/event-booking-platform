...
export default function VendorLoginPage() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-10 px-6 relative overflow-hidden">
            ...
            <motion.div
                className="grid grid-cols-2 max-md:grid-cols-1 w-full max-w-[960px] bg-bg-card border border-border-default rounded-3xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] relative z-[2]"
            ...
            >
                ...
                <div className="p-12 px-10 bg-gradient-to-br from-accent-emerald to-teal-500 flex flex-col justify-center max-md:hidden">
                    <div className="mb-10">
                        ...
                        <p className="text-white/75 text-[0.95rem]">Venue Owner Portal</p>
                    </div>
                ...
                <div className="p-12 px-10 max-md:p-9 max-md:px-6 flex flex-col justify-center">
                    <div className="mb-6">
                        <div className="w-14 h-14 flex items-center justify-center rounded-2xl bg-accent-emerald/15 text-accent-emerald text-2xl mb-4">
                            <HiOfficeBuilding />
                        </div>
                    ...
                    {error && (
                        <motion.div className="p-3 mb-4 bg-accent/10 border border-accent/20 rounded-xl text-accent text-sm" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
                            {error}
                        </motion.div>
                    )}
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-3.5 mb-5">
                        {!isLogin && (
                            <>
                                <div className={inputCls}>
                                    ...
                                    <input type="text" placeholder="Full Name / Business Name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required className={inputInner} />
                                </div>
                                <div className={inputCls}>
                                    <HiPhone className="text-text-muted text-lg shrink-0" />
                                    <input type="tel" placeholder="Mobile Number" value={formData.mobile} onChange={(e) => setFormData({ ...formData, mobile: e.target.value })} required className={inputInner} />
                                </div>
                            </>
                        )}
                        ...
                        <div className={inputCls}>
                            ...
                            <input type="email" placeholder="Email Address" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required className={inputInner} />
                        </div>
                        ...
                        {isLogin && (
                            <div className="flex items-center justify-between">
                                ...
                                <Link href="/forgot-password" className="text-[0.825rem] text-accent-emerald font-medium">Forgot password?</Link>
                            </div>
                        )}
                    ...
                    <div className="flex justify-center gap-6 text-sm">
                        <Link href="/admin" className="text-text-secondary hover:text-white transition-all duration-300">Admin Login →</Link>
                        <Link href="/login" className="text-text-secondary hover:text-white transition-all duration-300">Customer Login →</Link>
                    </div>
                </div>
            </motion.div>
        </div>
