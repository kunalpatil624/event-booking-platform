            ...
            <main className="pt-[100px] min-h-screen pb-20">
                <div className="max-w-[1280px] mx-auto px-6">
                    ...
                    <div className="flex items-center gap-2 text-sm text-text-muted mb-6">
                        ...
                        <span>/</span>
                        <Link href={`/venues?city=${venue.city}`} className="hover:text-white transition-colors">{venue.city}</Link>
                        <span>/</span>
                    ...
                    <div className="mb-8">
                        <div className="relative rounded-2xl overflow-hidden h-[450px] max-md:h-[280px]">
                            ...
                            <button className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-black/50 backdrop-blur-lg border-none rounded-full text-white text-xl hover:bg-black/70 transition-all duration-300" onClick={() => setCurrentImage(i => i > 0 ? i - 1 : venue.images.length - 1)}>
                                <HiChevronLeft />
                            </button>
                            <button className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-black/50 backdrop-blur-lg border-none rounded-full text-white text-xl hover:bg-black/70 transition-all duration-300" onClick={() => setCurrentImage(i => i < venue.images.length - 1 ? i + 1 : 0)}>
                                <HiChevronRight />
                            </button>
                        ...
                        <div>
                            ...
                            <motion.div className="mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                                <div className="flex items-start justify-between gap-4 mb-4 max-md:flex-col">
                                    <div>
                                        ...
                                        <div className="flex items-center gap-1.5 text-text-secondary text-sm">
                                            <HiLocationMarker className="text-accent" /> {venue.address}
                                        </div>
                                    </div>
                                ...
                                <div className="flex items-center gap-5 flex-wrap">
                                    <div className="flex items-center gap-1.5 text-sm font-semibold">
                                        ...
                                        <span className="text-white">{venue.rating.average}</span>
                                        <span className="text-text-muted font-normal">({venue.rating.count} reviews)</span>
                                    </div>
                            ...
                            <div className="flex gap-1 border-b border-border-default mb-6 overflow-x-auto">
                                {['overview', 'packages', 'menu', 'reviews', 'faqs'].map(tab => (
                                    <button key={tab} className={`px-5 py-3 text-sm font-medium border-b-2 transition-all duration-300 capitalize whitespace-nowrap ${activeTab === tab ? 'border-primary text-white' : 'border-transparent text-text-muted hover:text-text-secondary'}`} onClick={() => setActiveTab(tab)}>
                                        {tab}
                                    </button>
                                ))}
                            </div>
                            ...
                            {activeTab === 'overview' && (
                                <div className="space-y-8">
                                    <div>
                                        <h3 className="text-lg font-semibold text-white mb-3">About This Venue</h3>
                                        <p className="text-text-secondary text-sm leading-relaxed">{venue.description}</p>
                                    </div>
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 max-sm:grid-cols-1 gap-3">
                                            {amenitiesList.map(a => (
                                                <div key={a.key} className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border text-sm ${a.available ? 'bg-accent-emerald/[0.06] border-accent-emerald/15 text-white' : 'bg-white/[0.02] border-border-default text-text-muted line-through'}`}>
                                                    <HiCheck className={a.available ? 'text-accent-emerald' : 'text-text-muted'} />
                                                    <span>{a.label}</span>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {venue.occasions.map(o => (
                                                <span key={o} className="px-4 py-1.5 bg-primary/10 border border-primary/20 rounded-full text-primary-light text-sm font-medium capitalize">{o}</span>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <div key={i} className={`relative p-6 bg-bg-card border rounded-2xl cursor-pointer transition-all duration-300 ${selectedPackage === i ? 'border-primary shadow-[0_0_20px_rgba(108,60,225,0.15)]' : 'border-border-default hover:border-border-light'}`} onClick={() => setSelectedPackage(i)}>
                                                ...
                                                <ul className="space-y-2 mb-4">
                                                    {pkg.includes.map((item, j) => (
                                                        <li key={j} className="flex items-center gap-2 text-text-secondary text-sm"><HiCheck className="text-accent-emerald shrink-0" /> {item}</li>
                                                    ))}
                                                </ul>
                                                <button className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${selectedPackage === i ? 'bg-gradient-to-r from-primary to-primary-light text-white' : 'bg-white/[0.06] border border-border-default text-text-secondary hover:text-white hover:border-border-light'}`}>
                                            ...
                                            <div key={i} className="p-5 bg-bg-card border border-border-default rounded-2xl">
                                                <div className="flex items-center justify-between mb-3">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <h4 className="text-sm font-semibold text-white">{r.user}</h4>
                                        ...
                                        {venue.faqs.map((faq, i) => (
                                            <div key={i} className="bg-bg-card border border-border-default rounded-xl overflow-hidden">
                                                <button className="w-full flex items-center justify-between p-4 bg-transparent text-left text-white text-sm font-medium hover:bg-white/[0.02] transition-all duration-300" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                                                    <span>{faq.q}</span>
                                                    <span className="text-primary-light text-lg ml-4">{openFaq === i ? '−' : '+'}</span>
                                                </button>
                                                {openFaq === i && (
                                                    <motion.div className="px-4 pb-4" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                            ...
                            <div className="sticky top-[100px] bg-bg-card border border-border-default rounded-2xl p-6 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                                ...
                                <div className="space-y-3 mb-5">
                                    <div>
                                        ...
                                        <input type="date" className="w-full px-3 py-2.5 bg-bg-secondary border border-border-default rounded-xl text-white text-sm outline-none focus:border-primary transition-all duration-300" />
                                    </div>
                                    <div>
                                        ...
                                        <select className="w-full px-3 py-2.5 bg-bg-secondary border border-border-default rounded-xl text-white text-sm outline-none focus:border-primary transition-all duration-300 [&>option]:bg-bg-card">
                                            <option>Wedding</option><option>Reception</option><option>Engagement</option><option>Birthday</option><option>Corporate</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-text-muted mb-1.5 uppercase tracking-wide">Guest Count</label>
                                        <input type="number" placeholder="No. of guests" min="1" className="w-full px-3 py-2.5 bg-bg-secondary border border-border-default rounded-xl text-white text-sm outline-none placeholder:text-text-muted focus:border-primary transition-all duration-300" />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2.5 pt-4 border-t border-border-default">
                                    {['Instant Confirmation', 'Free Cancellation', 'Best Price Guarantee'].map(t => (
                                        <div key={t} className="flex items-center gap-2 text-text-secondary text-xs"><HiCheck className="text-accent-emerald" /> {t}</div>
                                    ))}
                                </div>
                            </div>
                        </div>
